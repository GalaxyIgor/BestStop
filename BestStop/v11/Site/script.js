    ...
    const sectors = [
        {
            ...
            occupancyRate: Math.round((Math.round(apiData.vagas_ocupadas ) / Math.round(apiData.total_vagas)) * 100),
            status: (Math.round(apiData.vagas_ocupadas) / Math.round(apiData.total_vagas)) > 0.7 ? 'high' :
...
function updateStatusDisplay() {
    ...
    // Atualiza apenas a taxa de ocupação
    ...
    // Mantém a atualização de horário
    if (parkingState.lastUpdate && elements.lastUpdate) {
...
function updateSectors() {
    ...
    parkingState.sectors.forEach(sector => {
        ...
        // Cria retângulo do setor com tooltip corrigido
        L.rectangle(bounds, {
            color: color,
            fillColor: color,
            fillOpacity: opacity,
            ...
            Vagas: ${Math.round(sector.occupied)}/${Math.round(sector.total_spots)}<br>
            ${Math.round((sector.occupied/sector.total_spots)*100)}% ocupado
        `, {
            direction: 'top',
            className: 'sector-tooltip' // opcional: para estilizar o tooltip
