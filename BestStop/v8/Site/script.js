...
// Mapeamento de fotos para setores
const fotoParaSetor = {
    "Foto1.jpg": "A",
    "Foto2.jpg": "B",
    "Foto3.jpg": "C",
...
function processApiData(apiData) {
    const setorAtual = fotoParaSetor[apiData.imagem] || "A";

    // Atualiza apenas o setor correspondente à foto
    const sectors = parkingState.sectors.map(sector => {
        if (sector.sector === setorAtual) {
            return {
                ...sector,
                total_spots: apiData.total_vagas,
                occupied: apiData.vagas_ocupadas,
                available: apiData.vagas_livres,
                occupancyRate: apiData.perc_ocupadas,
                status: apiData.perc_ocupadas > 70 ? 'high' :
                       apiData.perc_ocupadas > 40 ? 'medium' : 'low',
                image: apiData.imagem
            };
        }
        return sector; // Mantém os outros setores inalterados
    });

    // Se for a primeira vez ou o setor não existir
    if (sectors.length === 0 || !sectors.some(s => s.sector === setorAtual)) {
        const posicoesSetores = {
            "A": { x1: 30, y1: 570, x2: 170, y2: 20 },
            "B": { x1: 170, y1: 570, x2: 310, y2: 20 },
            "C": { x1: 490, y1: 570, x2: 630, y2: 20 },
            "D": { x1: 630, y1: 570, x2: 770, y2: 20 }
        };

        sectors.push({
            sector: setorAtual,
            position: posicoesSetores[setorAtual],
            total_spots: apiData.total_vagas,
            occupied: apiData.vagas_ocupadas,
            available: apiData.vagas_livres,
            occupancyRate: apiData.perc_ocupadas,
    ...
    return {
        totalSpots: sectors.reduce((sum, s) => sum + s.total_spots, 0),
        occupied: sectors.reduce((sum, s) => sum + s.occupied, 0),
        available: sectors.reduce((sum, s) => sum + s.available, 0),
        lastUpdate: new Date().toISOString(),
        sectors: sectors,
        currentImage: apiData.imagem
    };
}
...
function updateParkingState(data) {
    ...
    const totalSpots = data.sectors.reduce((sum, s) => sum + s.total_spots, 0);
    const occupied = data.sectors.reduce((sum, s) => sum + s.occupied, 0);
    const available = data.sectors.reduce((sum, s) => sum + s.available, 0);

    // Atualiza apenas se houver mudanças relevantes
    parkingState = {
        ...parkingState,
        totalSpots,
        occupied,
    ...
    // Mostra qual setor foi atualizado
    const setorAtualizado = fotoParaSetor[data.currentImage] || "A";
    showNotification(`Setor ${setorAtualizado} atualizado com a imagem ${data.currentImage}`, 'info');
}
